...
etics:
	...
	test ! -f $(rpmbuild_dir)/SRPMS/$(rpm_name)-$(version)-*.src.rpm || cp -v $(rpmbuild_dir)/SRPMS/$(rpm_name)-$(version)-*.src.rpm $(rpm_dir)
	if [ -f $(rpmbuild_dir)/RPMS/*/$(rpm_name)-$(version)-*.rpm ] ; then \
		cp -v $(rpmbuild_dir)/RPMS/*/$(rpm_name)-$(version)-*.rpm $(rpm_dir) ; \
		test ! -d $(tmp_dir) || rm -fr $(tmp_dir) ; \
		mkdir -p $(tmp_dir) ; \
		cd $(tmp_dir) ; \
